...
export default function StatementView() {
  ...
  const [statement, loading] = useDocumentData(docRef);

  if (!id) return null;
  if (loading) return <p className="p-4">Loading…</p>;
  if (!statement) return <p className="p-4">Statement not found.</p>;

  const mailto = statement.email ? `mailto:${statement.email}?subject=Statement%20${docId}` : null;
    ...
    <div className="min-h-screen flex flex-col bg-white text-black">
      ...
      <main className="flex-grow p-4 max-w-3xl mx-auto space-y-6">
        <div className="space-y-2">
          ...
          {statement.created && (
            <p><span className="font-medium">Created:</span> {new Date(statement.created.seconds * 1000).toLocaleString()}</p>
          )}
        </div>
        {statement.text && (
          <pre className="whitespace-pre-wrap border p-4 rounded bg-gray-100">
            {statement.text}
          </pre>
        )}
